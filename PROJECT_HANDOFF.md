...
- KPI 下方已补入信息卡：HA 主格局展示 `交联填充类 / 非交联水光、肤质改善类 / 含麻候选`，其中 `含麻候选` 是线索口径卡，不并入产品形态分类。
- 原 `厂家竞争力 + 产品形态结构` 双卡区域已删除，避免与头部卡片和后续业务矩阵重复。
- 新增 `交联填充剂定位拆分` 模块：
  - 地区/定位层级基于注册人、集团和产品名文本映射，不代表注册人法定住所字段。
  - 当前交联填充剂结构：国产 36、欧美进口 28、韩国进口 23、港澳台 3、其他进口 1。
  - 含麻严格口径：欧美进口 9、韩国进口 2、国产 2、其他进口 1。
  - 含麻候选口径：欧美进口 14、韩国进口 7、国产 2、其他进口 1。
  - 记录浏览器新增 `形态`、`定位`、`含麻状态` 筛选，并支持 URL 参数 `shape`、`position`、`lidocaine`。
...
2. 后续源表最好补 `country_or_region`、`position_tier`、`lidocaine_signal_status` 字段，把 HA 进口国家/地区和含麻候选规则从前台映射下沉到数据层。
