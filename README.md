...
- 统一使用 `HA/透明质酸钠`，不要混用 `玻尿酸`、`玻尿酸/透明质酸钠`。
- 少女针、羟基磷酸钙等页面表达不要使用“再生”，统一按更学术的表达处理，如 `胶原刺激剂`。
- 目前只打磨浅色模板，不做深色模板。
...
- 首页脚本：`current-refresh-62`
- 子赛道页 `track.js`：`current-refresh-62`
...
- 二类医疗器械不纳入主格局分析。
- `新型材料` 只指真实的新型材料，目前不是把 EBD 或其他非材料项目并入其中。
- EBD 不是材料，不进入材料家族统计。
